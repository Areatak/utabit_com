<div class="wrap">
	...
	<h2>Flipbooks
		<a href='<?php echo $url .'&action=add_new'; ?>' class='add-new-h2'><?php _e('Add New','cup_code');?></a>
	</h2>
	...
	<?php
		...
		foreach ($ids as $id) {
			...
			echo '<div id="message" class="updated notice is-dismissible below-h2">
					<p><strong>'.$prefix .' </strong><i>' . $names.'</i> <strong>deleted</strong>. <a class="undo" href="#"></a></p>
...
<thead>
			<tr>
				...
				<th style="width:150px"><?php _e('Shortcode','cup_code');?></th>
...
<input type="submit" name="filter_action" id="post-query-submit" class="button" value="Filter">		</div>-->
	...
	<br/>
	<br/>
	<h3><?php _e('Import / Export','cup_code');?></h3>
	<div>
		<a class='button-secondary' href='<?php echo admin_url( "admin.php?page=real3d_flipbook_admin&action=generate_json" ); ?>'><?php _e('Export (Generate JSON)','cup_code');?></a>
	</div>
	...
	<form method="post" enctype="multipart/form-data" action="admin.php?page=real3d_flipbook_admin&amp;action=import_from_json_confirm">
		...
		<?php 
			...
			<p class="submit"><input type="submit" name="submit" id="submit" class="button save-button button-secondary" value="دررون ریزی"></p>
	
	</form>
	<br/>
	<br/>
	<br/>
	<span class="submitbox"><a class="submitdelete" href='<?php echo $url .'&action=delete_all'; ?>'><?php _e('Delete all flipbooks','cup_code');?></a></span>
