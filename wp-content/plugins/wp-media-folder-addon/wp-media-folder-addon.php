...
/*
  ...
  Description: WP media Folder Addon is a WordPress plugin that enhance the WordPress media manager by adding a folder manager inside.
  Author: Joomunited
  Version: 1.0.3
...
if (version_compare(PHP_VERSION, '5.3', '<')) {
    ...
    if( !function_exists('wpmf_addon_show_error') ){
        function wpmf_addon_show_error(){
            echo '<div class="error"><p><strong>WP Media Folder Addon</strong> need at least PHP 5.3 version, please update php before installing the plugin.</p></div>';
        }
    }
...
add_action('init', function(){
    load_plugin_textdomain('wpmfAddon', false, dirname(plugin_basename(__FILE__)) . DIRECTORY_SEPARATOR . 'languages' .DIRECTORY_SEPARATOR);
}, 1);
    ...
    if(isset($_GET['function'])){
        switch ($_GET['function']){
            case 'wpmf_authenticated':
                $wpmfgoogleaddon->wpmf_authenticated();
                break;
            
            case 'wpmf_gglogout':
                $wpmfgoogleaddon->wpmf_gglogout();
                break;
