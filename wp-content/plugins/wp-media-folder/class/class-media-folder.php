...
class Wp_Media_Folder {
    ...
    function __construct() {
        ...
        add_action( 'wpml_media_create_duplicate_attachment', array($this, "wpmf_media_set_files_to_folder_wpml") );

    }

    public function wpmf_media_set_files_to_folder_wpml(){
            ...
            foreach ($posts as $post){
                ...
                if(!empty($terms)){
                    foreach ($terms as $term){
                        $this->wpmf_add_attachment_langguages($post->ID, $term->term_id, WPMF_TAXO);
                    }
                }
            }
        }
    ...
    public function load_script(){
        ...
        wp_enqueue_script('wpmf-assing-tree', plugins_url('/assets/js/assign_image_folder_tree.js', dirname(__FILE__)), array('jquery'), WPMF_VERSION);
    ...
    public function wpmf_term_root() {
        global $current_user;
        $term_roots = get_categories(array('taxonomy' => WPMF_TAXO ,'parent' => 0, 'hide_empty' => false));
        $wpmfterm = array();
    ...
    public function wpmf_get_assign_tree() {
        ...
        // Retrieve the terms in a given taxonomy or list of taxonomies.
        $categories = get_categories(array('taxonomy' => WPMF_TAXO,'orderby' => $orderby, 'order' => $order, 'parent' => $id, 'hide_empty' => false));
        $wpmf_active_media = get_option('wpmf_active_media');
        $wpmf_create_folder = get_option('wpmf_create_folder');
        $user_roles = $current_user->roles;
        $role = array_shift($user_roles);
    ...
    public function check_checked($term_id,$term_of_file){
        ...
        $pchecked = false;
        foreach ($childs as $child){
            if(in_array($child,$term_of_file)){
                $pchecked = true;
                break;
            }else{
                $pchecked = false;
