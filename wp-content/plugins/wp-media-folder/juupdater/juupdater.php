...
if (!class_exists('Ju_Updater', false)):
    ...
    class Ju_Updater {
        ...
        protected function installHooks() {
            ...
            add_action('admin_enqueue_scripts', array($this, 'ju_load_custom_wp_admin_script'), 0);
            add_action('wp_ajax_ju_add_token', array($this, 'ju_add_token'));
            add_action('wp_ajax_ju_logout', array($this, 'ju_logout'));


            add_action('admin_init', array($this, 'ju_add_button'));
        }

        public function ju_add_button() {
            global $pagenow;
            if ($pagenow == 'options-general.php') {
                ...
                add_settings_field('joom_live_update', 'Live updates status', array($this, 'joom_connect'), 'general', 'juupdater-connect');
        ...
        public function joom_connect() {
            $button_connect = isset($this->settings['joom_live_update']) ? $this->settings['joom_live_update'] : '0';
            ...
            if (empty($token)) {
                ...
                $button = '<span class="button button-primary ju-btn-disconnect">Disconnect my Joomunited account</span>';
            }
            echo $button;
        }
        ...
        public function ju_logout() {
            update_option('ju_user_token', '');
        }

        public function ju_load_custom_wp_admin_script() {
            global $pagenow;
            if ($pagenow == 'plugins.php' || $pagenow == 'options-general.php') {
                wp_register_script('check_token', plugins_url('/juupdater/js/check_token.js', dirname(__FILE__)), array(), $this->juversion);
                wp_enqueue_script('check_token');
            }

            if ($pagenow == 'update-core.php') {
                wp_register_script('ju_update_core', plugins_url('/juupdater/js/ju_update_core.js', dirname(__FILE__)), array(), $this->juversion);
                wp_enqueue_script('ju_update_core');
            }
            
            $params = $this->localize_script();
        ...
        public function requestInfo($queryArgs = array()) {
            ...
            if (!is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body'])) {
                $pluginInfo = Ju_PluginInfo::fromJson($result['body'], $this->debugMode);
        ...
        public function requestUpdate() {
            ...
            return Ju_PluginUpdate::fromPluginInfo($pluginInfo);
        }
        ...
        public function maybeCheckForUpdates() {
            ...
            $shouldCheck = apply_filters(
                    'puc_check_now-' . $this->slug, $shouldCheck, (!empty($state) && isset($state->lastCheck)) ? $state->lastCheck : 0, $this->checkPeriod
            );
        ...
        public function getUpdateState() {
            ...
            if (!empty($state) && isset($state->update) && is_object($state->update)) {
                $state->update = Ju_PluginUpdate::fromObject($state->update);
            }
            return $state;
        }
        ...
        public function injectUpdate($updates) {
            ...
            if (!empty($update)) {
                ...
                add_action("after_plugin_row_{$pluginFile}", array($this, 'ju_plugin_update_row'), 10, 2);
        ...
        function ju_plugin_update_row($file, $plugin_data) {
            $token = get_option('ju_user_token');
            if (!empty($token)) {
                ...
                if ($plugin_data['slug'] == 'wp-frontpage-news-pro-addon') {
                    $response = wp_remote_get(JU_BASE . 'index.php?option=com_juupdater&task=download.checktoken&extension=wp-latest-posts-addon.zip&token=' . $token);
                } else {
                    $response = wp_remote_get(JU_BASE . 'index.php?option=com_juupdater&task=download.checktoken&extension=' . $plugin_data['slug'] . '.zip&token=' . $token);
                }
                ...
                if (is_network_admin() || !is_multisite()) {
                    ...
                    if (!empty($a_body)) {
                        if ($a_body->status == false && $a_body->linkdownload != '') {
                            echo '<tr class="plugin-update-tr' . $active_class . '" id="ju-' . esc_attr($r->slug . '-update') . '" data-slug="' . esc_attr($r->slug) . '" data-plugin="' . esc_attr($file) . '"><td colspan="' . esc_attr($wp_list_table->get_column_count()) . '" class="plugin-update colspanchange"><div class="update-message">';
        ...
        public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
            ...
            if ($isRelevant && current_user_can('update_plugins')) {
                ...
                $link = JU_BASE . "index.php?option=com_juupdater&view=login&tmpl=component&site=" . admin_url() . "&TB_iframe=true&width=300&height=305";

                if (empty($ju_token)) {
                    $pluginMeta[] = sprintf('<a class="thickbox ju_update" href="%s">%s</a>', esc_attr($link), 'JoomUnited Login');
                }
            }
            return $pluginMeta;
        }
...
if (!class_exists('Ju_PluginInfo', false)):
...
if (!class_exists('Ju_PluginUpdate', false)):
    ...
    class Ju_PluginUpdate {
        ...
        public static function fromJson($json, $triggerErrors = false) {
            ...
            $pluginInfo = Ju_PluginInfo::fromJson($json, $triggerErrors);
            if ($pluginInfo != null) {
